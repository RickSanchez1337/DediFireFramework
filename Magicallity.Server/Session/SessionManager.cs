...
namespace Magicallity.Server.Session
{
    public class SessionManager : ServerAccessor
    {
        ...
        private void OnInitialConnect([FromSource] Player source, string playerName, dynamic setKickReason, dynamic d)
        //public void OnQueueConnection(Session playerSession, dynamic nope)
        {
            if (string.IsNullOrEmpty(source.Identifiers["steam"]))
            {
                ...
                setKickReason("Steam must be running to join this server. Please restart Steam and FiveM and try and join again");
                return;
            }
